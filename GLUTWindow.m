...
- (void)windowDidChangeBackingProperties:(NSNotification *)notification {
    CGFloat oldBackingScaleFactor = [[[notification userInfo]
         ...
         doubleValue];
    CGFloat newBackingScaleFactor = [self backingScaleFactor];
    CGFloat ratio = oldBackingScaleFactor/newBackingScaleFactor;
    NSRect frame = self.frame;
    NSRect bounds = [self.contentView bounds];
    frame.size.width  += bounds.size.width *(ratio-1);
    frame.size.height += bounds.size.height*(ratio-1);
    frame.origin.y -= bounds.size.height*(ratio-1)/2;
    frame.origin.x -= bounds.size.width*(ratio-1)/2;
