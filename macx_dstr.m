      ...
      switch(criteria[i].capability) {
         ...
         case NO_RECOVERY:
          ...
          case PROFILE_3_2_CORE:
              if(weightForCriterion(&criteria[i], 0, 1) > 0) {
                list[n++] = NSOpenGLPFAOpenGLProfile;
                list[n++] = NSOpenGLProfileVersion3_2Core;
              }
              break;
      ...
      case 'c':
         if (!strcmp(word, "conformant")) {
            ...
            *mask |= (1 << CONFORMANT);
            return 1;
         // Alec:
         }else if(!strcmp(word, "core")) {
             criterion[0].capability = PROFILE_3_2_CORE;
             if (comparator == CMP_NONE) {
                 criterion[0].comparison = CMP_EQ;
                 criterion[0].value = 1;
             } else {
